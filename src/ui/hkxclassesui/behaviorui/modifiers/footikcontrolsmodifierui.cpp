...
void FootIkControlsModifierUI::loadData(HkxObject *data){
    ...
    if (data){
        if (data->getSignature() == HKB_FOOT_IK_CONTROLS_MODIFIER){
            ...
            UIHelper::loadBinding(ON_OFF_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/onOffGain", table, bsData);
            UIHelper::loadBinding(GROUND_ASCENDING_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/groundAscendingGain", table, bsData);
            UIHelper::loadBinding(GROUND_DESCENDING_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/groundDescendingGain", table, bsData);
            UIHelper::loadBinding(FOOT_PLANTED_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/footPlantedGain", table, bsData);
            UIHelper::loadBinding(FOOT_RAISED_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/footRaisedGain", table, bsData);
            UIHelper::loadBinding(FOOT_UNLOCK_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/footUnlockGain", table, bsData);
            UIHelper::loadBinding(WORLD_FROM_MODEL_FEEDBACK_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/worldFromModelFeedbackGain", table, bsData);
            UIHelper::loadBinding(ERROR_UP_DOWN_BIAS_ROW, BINDING_COLUMN, varBind, "controlData/gains/errorUpDownBias", table, bsData);
            UIHelper::loadBinding(ALIGN_WORLD_FROM_MODEL_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/alignWorldFromModelGain", table, bsData);
            UIHelper::loadBinding(HIP_ORIENTATION_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/hipOrientationGain", table, bsData);
            UIHelper::loadBinding(MAX_KNEE_ANGLE_DIFFERENCE_ROW, BINDING_COLUMN, varBind, "controlData/gains/maxKneeAngleDifference", table, bsData);
            UIHelper::loadBinding(ANKLE_ORIENTATION_GAIN_ROW, BINDING_COLUMN, varBind, "controlData/gains/ankleOrientationGain", table, bsData);
